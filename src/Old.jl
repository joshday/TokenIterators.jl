...
Next(t::Token) = Next(t.data, @view(t.data[t.j+1:end]), StringView(@view(t.data[t.j+1:end])), t.j+1, t.kind)
Base.String(s::Next) = String(s.view)
function Base.show(io::IO, t::Next)
    k = styled"{bright_red:Green} {bright_yellow:$(format(t.i))} {bright_cyan:prev=$(repr(t.kind))} "
    n = displaysize(io)[2] - length(k)
    print(io, k, styled"{bright_black:$(limit_string(repr(String(t)), n))}")
end

firstchar(x::Next) = first(x.sview)
...
function Base.iterate(t::Token, state=Next(t))
    Base.isdone(t, state) && return nothing
    n = next(t.domain, state)
    return n, Next(n)
end
...
→(s::Next, x) = is_token(s, x)
→(data::AbstractVector{UInt8}, x) = is_token(data, x)

#-----------------------------------------------------------------------------# token_end
struct Before{T} x::T end
token_end(s::Next, b::Before) = (j = token_end(s, b.x); isnothing(j) ? length(s.data) : j - 1)

struct After{T} x::T end
token_end(s::Next, a::After) = token_end(s, a.x) + 1
...
function token_end(s::Next, o::Unescaped)
    (; data, i) = s
    skip = false
...
token_end(s::Next, x::UInt8) = findnext(==(x), s.data, s.i)
...
is_token(s::Next, r::Rule) = is_token(s, r.id)
...
is_token(s::Next, x) = is_token(s.view, x)
is_token(s::AbstractVector{UInt8}, x) = is_token(first(s), x)

is_token(s::UInt8, x::UInt8) = s == x
is_token(s::UInt8, x::Char) = Char(s) == x
is_token(s::UInt8, x::Set) = any(x -> is_token(s, x), x)
is_token(s::UInt8, x::Function) = hasmethod(x, (UInt8,)) ? x(s) : x(Char(s))
...
get_val(o::Partition{<:Set}, x::UInt8) = is_token(o.x, x)
...
#-----------------------------------------------------------------------------# BSplit
struct BSplit{T} <: TokenDomain
    rule::T
end
init(o::BSplit) = false

function next(o::BSplit, state::Next)
    if is_token(state, o.rule)
        return Token(o, state.data, true, state.i, token_end(state, o.rule))
    else
        return Token(o, state.data, false, state.i, token_end(state, Before(o.rule)))
    end
end
...
# struct Rule{T, S}
#     is_token::T   # passed to `startswith(is_token, state)`
